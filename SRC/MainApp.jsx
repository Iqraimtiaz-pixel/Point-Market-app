  ...
  return (
    <div className={`kt-root ${darkMode ? "kt-dark" : ""}`}>
      ...
      <div className="kt-phone">
        ...
        {/* ── MAIN TABS ── */}
        {!screen && tab === "home"    && <HomeScreen    userLocation={userLocation} currentUser={currentUser} onOpenDetail={openDetail} onTrade={(i) => { setActiveItem(i); setTradeSheet(true); }} onOpenMap={() => setScreen("map")} onOpenProfile={() => { setTab("profile"); setScreen(null); }} onOpenPmSpace={openPmSpace} onOpenCreate={() => { setTab("create"); setScreen(null); }} onOpenProducts={(cat) => { setSelectedCategory(cat || null); setScreen("products"); }} onOpenSkills={(cat) => { setSelectedCategory(cat || null); setScreen("skills"); }} />}
        ...
        {screen === "products" && <MarketplaceScreen filter="products" initialCategory={selectedCategory} userLocation={userLocation} currentUser={currentUser} onBack={closeScreen} onOpenDetail={openDetail} />}
        {screen === "skills"   && <MarketplaceScreen filter="skills"   initialCategory={selectedCategory} userLocation={userLocation} currentUser={currentUser} onBack={closeScreen} onOpenDetail={openDetail} />}
        {screen === "saved"    && <SavedScreen   onBack={closeScreen} onOpenDetail={openDetail} currentUser={currentUser} />}
        {screen === "settings" && <SettingsScreen onBack={closeScreen} userLocation={userLocation} onUpdateLocation={async (loc) => { setUserLocation(loc); if (currentUser?.uid) { try { await updateUserProfile(currentUser.uid, { city: loc.city, completeAddress: loc.completeAddress ?? "", lat: loc.lat, lng: loc.lng }); } catch (e) { console.warn("Failed to persist location:", e); } } }} onLogout={onLogout} darkMode={darkMode} onToggleDarkMode={toggleDarkMode} currentUser={currentUser} onProfileUpdate={onProfileUpdate} />}
        ...
        {/* ── BOTTOM NAV ── */}
        {(!screen || screen === "products" || screen === "skills") && <BottomNav tab={tab} setTab={(t) => { setTab(t); setScreen(null); }} />}
      </div>
    </div>
  );
}
