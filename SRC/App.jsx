  ...
  return (
    <div className={`kt-root ${darkMode ? "kt-dark" : ""}`}>
      ...
      <div className="kt-phone">
        ...
        {!screen && tab === "profile" && <ProfileScreen onNavigate={goTo} userLocation={userLocation} currentUser={currentUser} onOpenDetail={openDetail} onProfileUpdate={handleProfileUpdate} />}
    ...
    <div className="kt-scroll">
      ...
      <div className="pms-action-row">
        ...
        <button
          ...
          onClick={() => onOpenChat && onOpenChat({ id: displayId, user: displayUser, avatar: displayAvatar })}
        >
...
function ProfileScreen({ onNavigate, userLocation, currentUser, onOpenDetail, onProfileUpdate }) {
  ...
  const displayAvatarUrl = currentUser?.avatarUrl    || null; // PHASE 2 — real photo, falls back to emoji below
  const displayAvatar    = currentUser?.avatarEmoji  || "🧑‍🚀";
  const displayId        = (typeof currentUser?.uid === "string" && currentUser.uid.length >= 6)
                              ? `PM-${currentUser.uid.slice(0, 6).toUpperCase()}`
                              : "PM-000";
  ...
  // PHASE 2 — Edit Profile sheet toggle
  ...
  // ── ON-SCREEN DEBUG STEP TRACKER ──
  // Each major section is built into a local variable inside its own
  // try/catch BEFORE the return statement. If a section throws, we record
  // which step failed and substitute a safe fallback for that section only —
  // the debug box below always shows the last step actually reached.
  let debugStep = 0;
  let debugCrash = null;

  let headerSection;
  try {
    debugStep = 1; // STEP 1 — header (avatar, name, bio, PM ID, city, Edit button)
    headerSection = (
      <div className="profile-head">
        <div className="profile-avatar">
          {displayAvatarUrl ? <img src={displayAvatarUrl} alt="" className="profile-avatar-img" /> : displayAvatar}
        </div>
        ...
        <button className="kt-btn profile-edit-btn" onClick={() => setShowEdit(true)}><User size={15} /> Edit Profile</button>
      </div>
    );
  } catch (e) {
    debugCrash = { step: 1, message: e?.message || String(e) };
    headerSection = <div className="profile-head"><div className="empty-state">Header failed to render.</div></div>;
  }

  let statsSection;
  try {
    debugStep = 2; // STEP 2 — stats row (Posts / Karma / Trades / Followers / Following)
    statsSection = (
    ...
    debugCrash = debugCrash || { step: 2, message: e?.message || String(e) };
    statsSection = null;
  }

  let gridSection;
  try {
    debugStep = 3; // STEP 3 — content grid (Instagram-style post tiles)
    gridSection = (
      <div className="profile-grid">
        {myPostsError ? (
          ...
          const tileSrc = (post.mediaType === "video" ? (post.thumbnailUrl || post.mediaUrl) : (post.mediaUrl || post.thumbnailUrl)) || "";
          return (
            <div key={post.id} className="profile-grid-tile" onClick={() => onOpenDetail && onOpenDetail(post)}>
              {tileSrc && <img src={tileSrc} alt="" />}
    ...
    debugCrash = debugCrash || { step: 3, message: e?.message || String(e) };
    gridSection = <div className="empty-state">Content grid failed to render.</div>;
  }
  ...
  const debugBox = (
    <div style={{
      ...
      {`STEP 1${debugStep >= 1 ? " ✓" : ""}\nSTEP 2${debugStep >= 2 ? " ✓" : ""}\nSTEP 3${debugStep >= 3 ? " ✓" : ""}`}
      {debugCrash && `\nCRASH @ STEP ${debugCrash.step}: ${debugCrash.message}`}
    </div>
  );
...
function EditProfileSheet({ onClose, currentUser, onProfileUpdate }) {
  ...
  const [avatarUrl, setAvatarUrl] = useState(currentUser?.avatarUrl || null); // PHASE 2
  ...
  // PHASE 2 — profile picture upload via the existing Cloudinary service
  ...
  return (
    <div className="sheet-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="sheet">
        ...
        {/* PHASE 2 — profile picture picker */}
        <div className="edit-avatar-row">
          <div className="edit-avatar-preview" onClick={pickPicture}>
            {avatarUrl ? <img src={avatarUrl} alt="" onError={(e) => { e.currentTarget.style.display = "none"; }} /> : <span>{fallbackAvatar}</span>}
            <div className="edit-avatar-badge"><Camera size={13} /></div>
          </div>
...
const CSS = `
  ...
  /* ── PHASE 2: real profile picture + Edit Profile button ── */
  .profile-avatar-img { width: 100%; height: 100%; object-fit: cover; border-radius: 999px; }
  .profile-edit-btn { margin-top: 16px; }

  /* ── PHASE 2: Edit Profile — avatar picker ── */
  .edit-avatar-row { display: flex; flex-direction: column; align-items: center; gap: 10px; margin-bottom: 16px; }
  .edit-avatar-preview { position: relative; width: 84px; height: 84px; border-radius: 999px; background: var(--sea-light); display: flex; align-items: center; justify-content: center; font-size: 34px; overflow: hidden; cursor: pointer; }
  .edit-avatar-preview img { width: 100%; height: 100%; object-fit: cover; }
  .edit-avatar-badge { position: absolute; bottom: 0; right: 0; width: 26px; height: 26px; border-radius: 999px; background: var(--sea); color: #fff; display: flex; align-items: center; justify-content: center; border: 2px solid var(--bg); }
