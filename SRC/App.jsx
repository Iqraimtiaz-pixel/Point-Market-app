...
import {
  ...
  UserPlus, UserCheck, Info, Video, Film, Camera,
...
───────────────────────────────────────── */
function FeedCard({ item, onOpenDetail, onTrade, onOpenPmSpace, currentUser }) {
...
/* ─────────────────────────────────────────
   VIDEO DETAIL SCREEN
───────────────────────────────────────── */
function DetailScreen({ item, userLocation, onBack, onTrade, onAiHub, onBattle, onOpenPmSpace, currentUser }) {
  const [saved,           setSaved]           = useState(false);
  ...
  const saveToFirestore = async () => {
    ...
    try {
      ...
      await setDoc(userPostRef, {
        ...
        thumbnailUrl,
        mediaUrl:   postData.mediaUrl,   // was missing — video/image was unrenderable from this mirror doc
        publicId:   postData.publicId,   // was missing — no fallback thumbnail could be built either
  ...
  return (
    <div className="kt-scroll cp-upload-screen">
      ...
      <div className="cp-upload-body">
        {/* ── Media picker / preview — compact 16:9 rounded card ── */}
        {!mediaUrl ? (
      ...
      {/* ── Submit button — sticky to the bottom of the screen ── */}
...
───────────────────────────────────────── */
function InboxScreen({ onOpenChat, currentUser }) {
...
/* ─────────────────────────────────────────
   👤 PM SPACE — public seller profile screen
   Resolves `username` to a real Firestore user, then reads everything else
   live: profile fields from /users/{uid} (via useLiveProfileStats), and
   listings/videos counts from /posts (userId == uid).
───────────────────────────────────────── */
function PmSpaceScreen({ username, onBack, onOpenDetail, isFollowing, onToggleFollow, onOpenChat }) {
...
───────────────────────────────────────── */
/* ─────────────────────────────────────────
   🛡️ SAFETY NET — isolates newer, additive UI so a runtime error in it
   (e.g. a bad image URL, an unexpected Firestore shape) can never blank
   out the rest of the screen it's mounted in.
───────────────────────────────────────── */
function ProfileScreen({ onNavigate, userLocation, currentUser, onOpenDetail, onProfileUpdate }) {
...
/* ─────────────────────────────────────────
   MY LISTINGS
───────────────────────────────────────── */
function ListingsScreen({ onBack, currentUser, onOpenDetail }) {
  const uid = currentUser?.uid;
...
/* ─────────────────────────────────────────
   SAVED ITEMS
───────────────────────────────────────── */
function SavedScreen({ onBack, onOpenDetail, currentUser }) {
...
const CSS = `
  ...
  .kt-root {
    --sea: #1d4ed8; --sea-dark: #1e3a8a; --sea-light: #eef2ff;
    --pink: #e11d48; --pink-dark: #9f1239; --pink-light: #fff1f2;
    --ink: #0b1220; --muted: #64748b; --line: #edf0f5; --bg: #fff;
    min-height: 100vh; width: 100%;
    background: #f4f5f7; font-family: 'Inter', sans-serif;
  ...
  .kt-phone {
    ...
    box-shadow: 0 40px 90px -35px rgba(15,28,40,0.28), 0 4px 24px -8px rgba(15,28,40,0.12);
  ...
  /* ── TOP BAR ── */
  .topbar { display: flex; align-items: center; gap: 8px; padding: 12px 14px 10px; background: rgba(255,255,255,0.72); backdrop-filter: blur(14px) saturate(180%); -webkit-backdrop-filter: blur(14px) saturate(180%); position: sticky; top: 0; z-index: 5; }
  .pm-logo-mark { display: flex; align-items: center; font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 15px; flex-shrink: 0; letter-spacing: -0.03em; }
  .pm-logo-p { color: var(--sea); }
  .pm-logo-m { color: var(--pink); }
  .search-pill { flex: 1; display: flex; align-items: center; gap: 8px; background: var(--sea-light); border-radius: 999px; padding: 9px 14px; color: var(--muted); font-size: 13.5px; transition: background .2s ease; }
  .search-input { flex: 1; background: none; border: none; outline: none; font-family: inherit; font-size: 13.5px; color: var(--ink); }
  .search-input::placeholder { color: var(--muted); }
  .icon-btn { width: 36px; height: 36px; border-radius: 999px; border: 1px solid var(--line); background: var(--bg); display: flex; align-items: center; justify-content: center; color: var(--ink); flex-shrink: 0; cursor: pointer; transition: transform .15s ease, background .15s ease; }
  .icon-btn:active { transform: scale(0.92); background: var(--sea-light); }

  /* ── FEED CARD ── */
  .feed-card { margin: 8px 14px 14px; border-radius: 18px; overflow: hidden; border: 1px solid var(--line); background: var(--bg); box-shadow: 0 6px 20px -14px rgba(15,28,40,0.18), 0 1px 3px rgba(15,28,40,0.04); transition: transform .18s ease, box-shadow .18s ease; }

  .feed-video {
    height: 215px; position: relative; display: flex; align-items: center; justify-content: center; overflow: hidden;
  }
  ...
  /* ── TOP BADGE ROW — category, duration, demo grouped together, cleaner than scattered corners ── */
  .feed-top-badges { position: absolute; top: 10px; left: 10px; right: 10px; z-index: 2; display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
  .category-badge { background: rgba(255,255,255,0.9); backdrop-filter: blur(6px); color: var(--sea-dark); padding: 3px 10px; border-radius: 999px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; }
  .duration-badge { background: rgba(11,18,32,0.55); backdrop-filter: blur(6px); color: #fff; padding: 3px 9px; border-radius: 999px; font-size: 10px; font-weight: 600; display: inline-flex; align-items: center; }
  .demo-badge { background: rgba(225,29,72,0.92); backdrop-filter: blur(6px); color: #fff; padding: 3px 10px; border-radius: 999px; font-size: 9.5px; font-weight: 700; letter-spacing: 0.04em; }
  .feed-emoji { font-size: 60px; position: relative; z-index: 1; filter: drop-shadow(0 8px 18px rgba(15,28,23,0.15)); }
  ...
  /* ── SIDEBAR ACTIONS ── */
  .feed-actions { position: absolute; top: 16px; right: 16px; display: flex; flex-direction: column; align-items: center; gap: 12px; z-index: 3; }
  .act-wrap { display: flex; flex-direction: column; align-items: center; gap: 2px; }
  .act-btn { width: 32px; height: 32px; border-radius: 999px; background: rgba(255,255,255,0.94); border: none; display: flex; align-items: center; justify-content: center; box-shadow: 0 3px 10px rgba(15,28,40,0.14); cursor: pointer; backdrop-filter: blur(6px); transition: transform .15s cubic-bezier(.34,1.56,.64,1); }
  .act-btn:active { transform: scale(0.88); }
  .act-count { font-size: 9.5px; color: #fff; font-weight: 700; text-shadow: 0 1px 3px rgba(0,0,0,0.4); }

  /* ── PREMIUM TRADE FAB — the hero action, visually distinct from the rest ── */
  .trade-fab-wrap { display: flex; flex-direction: column; align-items: center; gap: 3px; margin-top: 2px; }
  .trade-fab { width: 42px; height: 42px; border-radius: 999px; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #22c55e, #15803d); box-shadow: 0 6px 18px -4px rgba(21,128,61,0.6), 0 0 0 3px rgba(255,255,255,0.5); transition: transform .15s cubic-bezier(.34,1.56,.64,1); }
  .trade-fab:active { transform: scale(0.88); }
  .trade-fab.disabled { opacity: 0.45; cursor: not-allowed; box-shadow: none; }
  .trade-fab-label { font-size: 9.5px; color: #fff; font-weight: 700; text-shadow: 0 1px 3px rgba(0,0,0,0.4); }
  ...
  .feed-user-row { display: flex; align-items: center; gap: 8px; margin-bottom: 7px; }
  .avatar-sm { width: 28px; height: 28px; border-radius: 999px; background: var(--sea-light); display: flex; align-items: center; justify-content: center; font-size: 13px; }
  .feed-username { font-weight: 600; font-size: 13px; letter-spacing: -0.01em; }
  .karma-pill { margin-left: auto; display: flex; align-items: center; gap: 3px; background: var(--sea-light); color: var(--sea-dark); font-size: 10.5px; font-weight: 700; padding: 3px 8px; border-radius: 999px; }
  .feed-title { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 15px; letter-spacing: -0.015em; margin: 0 0 3px; }
  .feed-desc { font-size: 12.5px; color: var(--muted); line-height: 1.5; margin: 0 0 9px; }
  .ai-row { display: flex; align-items: center; gap: 5px; font-size: 12px; font-weight: 700; color: var(--sea-dark); letter-spacing: -0.01em; }

  /* ── BOTTOM NAV ── */
  .bottom-nav { display: flex; border-top: 1px solid var(--line); background: rgba(255,255,255,0.78); backdrop-filter: blur(16px) saturate(180%); -webkit-backdrop-filter: blur(16px) saturate(180%); padding: 9px 8px 15px; }
  .nav-item { flex: 1; display: flex; flex-direction: column; align-items: center; gap: 3px; color: var(--muted); font-size: 9.5px; font-weight: 600; cursor: pointer; transition: color .2s ease, transform .15s ease; }
  .nav-item:active { transform: scale(0.92); }
  .nav-item.active { color: var(--sea); }
  .nav-item span { margin-top: 1px; }
  .nav-plus { width: 38px; height: 38px; border-radius: 13px; background: linear-gradient(135deg, var(--pink), var(--pink-dark)); box-shadow: 0 4px 14px -4px rgba(225,29,72,0.5); color: white; display: flex; align-items: center; justify-content: center; transition: transform .15s ease; }
  .nav-item:active .nav-plus { transform: scale(0.9); }

  /* ── SCREEN CHROME ── */
  .screen-header { display: flex; align-items: center; gap: 10px; padding: 13px 16px; border-bottom: 1px solid var(--line); background: rgba(255,255,255,0.78); backdrop-filter: blur(14px) saturate(180%); -webkit-backdrop-filter: blur(14px) saturate(180%); position: sticky; top: 0; z-index: 5; }
  .screen-header h2 { font-family: 'Space Grotesk', sans-serif; font-size: 16.5px; font-weight: 700; letter-spacing: -0.02em; margin: 0; }
  .back-btn-inline { width: 32px; height: 32px; border-radius: 999px; border: 1px solid var(--line); background: var(--bg); display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0; transition: transform .15s ease, background .15s ease; }
  .back-btn-inline:active { transform: scale(0.9); background: var(--sea-light); }
  .section-title { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 13.5px; letter-spacing: -0.01em; padding: 16px 16px 8px; }
  .empty-state { text-align: center; color: var(--muted); font-size: 13px; padding: 44px 24px; line-height: 1.6; }
  .feed-empty-pro { padding: 48px 24px; }
  .feed-empty-icon { width: 50px; height: 50px; margin: 0 auto 14px; border-radius: 16px; background: var(--sea-light); color: var(--sea-dark); display: flex; align-items: center; justify-content: center; }
  .feed-empty-title { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 15px; letter-spacing: -0.01em; color: var(--ink); margin-bottom: 5px; }
  .feed-empty-sub { font-size: 12.5px; color: var(--muted); margin-bottom: 18px; line-height: 1.5; }
  .feed-empty-cta { max-width: 240px; margin: 0 auto; }

  /* ── ITEM 10/11: LOADING SPINNER + SKELETON SHIMMER (previously undefined) ── */
  .feed-loading { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; padding: 56px 24px; color: var(--muted); font-size: 12.5px; }
  .feed-loading-spinner { width: 28px; height: 28px; border-radius: 999px; border: 2.5px solid var(--sea-light); border-top-color: var(--sea); animation: ktSpin .7s linear infinite; }
  @keyframes ktSpin { to { transform: rotate(360deg); } }
  @keyframes ktShimmer { 0% { background-position: -200% 0; } 100% { background-position: 200% 0; } }
  .skeleton { background: linear-gradient(90deg, var(--line) 25%, rgba(0,0,0,0.04) 37%, var(--line) 63%); background-size: 400% 100%; animation: ktShimmer 1.4s ease infinite; border-radius: 12px; }
  .skeleton-card { margin: 8px 14px 14px; border-radius: 18px; overflow: hidden; }
  .skeleton-card .skeleton-media { height: 232px; border-radius: 0; }
  .skeleton-card .skeleton-line { height: 12px; margin: 10px 14px 0; border-radius: 6px; }
  .skeleton-card .skeleton-line.w-60 { width: 60%; }
  .skeleton-card .skeleton-line.w-40 { width: 40%; margin-bottom: 14px; }

  /* ── SHEETS ── */
  .sheet-backdrop { position: absolute; inset: 0; background: rgba(11,18,32,0.4); backdrop-filter: blur(3px); z-index: 30; display: flex; align-items: flex-end; animation: fadeBd .22s ease-out; }
  .sheet-backdrop.nested { z-index: 60; background: rgba(11,18,32,0.55); }
  @keyframes fadeBd { from { opacity: 0; } }
  .sheet { width: 100%; background: var(--bg); border-radius: 22px 22px 0 0; padding: 8px 20px 28px; max-height: 80%; overflow-y: auto; box-shadow: 0 -12px 40px -12px rgba(15,28,40,0.2); animation: sheetUp .3s cubic-bezier(.22,1,.36,1); }
  @keyframes sheetUp { from { transform: translateY(100%); } }
  .sheet-handle { width: 36px; height: 4px; background: var(--line); border-radius: 99px; margin: 6px auto 14px; }
  .sheet-title { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 17px; letter-spacing: -0.015em; margin: 0 0 4px; }
  ...
  /* ── BUTTONS ── */
  .kt-btn { width: 100%; padding: 13.5px; border-radius: 14px; border: none; background: var(--sea); color: white; font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 13.5px; letter-spacing: -0.01em; display: flex; align-items: center; justify-content: center; gap: 8px; cursor: pointer; box-shadow: 0 6px 16px -6px rgba(29,78,216,0.45); transition: transform .12s ease, box-shadow .12s ease, opacity .12s ease; }
  .kt-btn:active { transform: scale(0.98); box-shadow: 0 3px 10px -6px rgba(29,78,216,0.4); }
  .kt-btn.ghost { background: var(--bg); color: var(--sea-dark); border: 1px solid var(--line); box-shadow: none; }
  .kt-btn:disabled { opacity: 0.4; cursor: not-allowed; }
  ...
  /* ── FORMS ── */
  .field-block { padding: 0 16px 13px; }
  .field-label { display: block; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: var(--muted); margin-bottom: 6px; }
  .field-input { width: 100%; border: 1.5px solid var(--line); border-radius: 12px; padding: 11px 14px; font-size: 13.5px; font-family: inherit; outline: none; transition: border-color .15s ease, box-shadow .15s ease; }
  .field-input:focus { border-color: var(--sea); box-shadow: 0 0 0 3px var(--sea-light); }
  .field-textarea { width: 100%; border: 1.5px solid var(--line); border-radius: 12px; padding: 11px 14px; font-size: 13.5px; font-family: inherit; outline: none; resize: none; min-height: 80px; transition: border-color .15s ease, box-shadow .15s ease; }
  .field-textarea:focus { border-color: var(--sea); box-shadow: 0 0 0 3px var(--sea-light); }
  ...
  /* ── CREATE POST — redesigned upload screen ── */
  .cp-upload-screen { display: flex; flex-direction: column; height: 100%; }
  .cp-upload-body { flex: 1; padding: 16px; padding-bottom: 8px; display: flex; flex-direction: column; gap: 16px; }
  .cp-preview-card { border-radius: 16px; box-shadow: 0 2px 12px rgba(0,0,0,0.07); overflow: hidden; position: relative; }
  .cp-preview-empty { aspect-ratio: 16 / 9; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 20px; background: var(--sea-light); cursor: pointer; border: 1.5px dashed var(--line); box-shadow: none; }
  .cp-preview-empty-title { font-weight: 700; margin-top: 8px; color: var(--ink); font-size: 14px; }
  .cp-preview-empty-sub { font-size: 12px; margin-top: 4px; color: var(--muted); max-width: 240px; }
  .cp-preview-empty-badge { display: flex; align-items: center; gap: 4px; font-size: 11px; margin-top: 10px; color: #22c55e; font-weight: 600; }
  .cp-preview-filled { aspect-ratio: 16 / 9; background: #000; }
  .cp-preview-media { width: 100%; height: 100%; object-fit: cover; display: block; }
  .cp-preview-replace { position: absolute; top: 10px; right: 10px; display: flex; align-items: center; gap: 4px; font-size: 11.5px; font-weight: 600; padding: 6px 12px; border-radius: 999px; background: rgba(255,255,255,0.92); color: var(--ink); border: none; }
  .cp-media-meta { display: flex; justify-content: space-between; align-items: center; margin-top: -8px; padding: 0 2px; }
  .cp-media-name { font-size: 12px; font-weight: 600; color: var(--ink); }
  .cp-media-sub { font-size: 11px; color: var(--muted); }
  .cp-fields-card { background: var(--bg); border-radius: 16px; box-shadow: 0 2px 12px rgba(0,0,0,0.06); padding: 16px; display: flex; flex-direction: column; gap: 14px; }
  .cp-fields-card .field-block { margin: 0; }
  .cp-info-card { background: var(--sea-light); border-radius: 16px; padding: 14px 16px; display: flex; flex-direction: column; gap: 10px; }
  .cp-info-row { display: flex; align-items: center; gap: 8px; font-size: 12.5px; color: var(--sea-dark); }
  .cp-submit-bar { position: sticky; bottom: 0; background: var(--bg); padding: 12px 16px calc(12px + env(safe-area-inset-bottom)); box-shadow: 0 -4px 14px rgba(0,0,0,0.07); }
  .cp-submit-bar .kt-btn { width: 100%; }
  ...
  /* ── CHAT ── */
  .chat-row { display: flex; align-items: center; gap: 12px; padding: 11px 16px; border-bottom: 1px solid var(--line); cursor: pointer; transition: background .15s ease; }
  .chat-row:active { background: var(--sea-light); }
  ...
  .msg-row { display: flex; padding: 3px 16px; }
  .msg-row.me { justify-content: flex-end; }
  .msg-bubble { max-width: 75%; padding: 9px 13px; border-radius: 16px; font-size: 13.5px; line-height: 1.4; box-shadow: 0 2px 8px -4px rgba(15,28,40,0.12); }
  ...
  .chat-input-row { display: flex; align-items: center; gap: 10px; padding: 11px 16px; border-top: 1px solid var(--line); background: rgba(255,255,255,0.85); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); }
  .chat-input { flex: 1; background: var(--sea-light); border: none; outline: none; border-radius: 999px; padding: 10px 16px; font-size: 13px; font-family: inherit; transition: box-shadow .15s ease; }
  .chat-input:focus { box-shadow: 0 0 0 3px rgba(29,78,216,0.15); }
  .send-btn { width: 36px; height: 36px; border-radius: 999px; background: var(--sea); color: white; display: flex; align-items: center; justify-content: center; flex-shrink: 0; cursor: pointer; border: none; box-shadow: 0 4px 12px -4px rgba(29,78,216,0.5); transition: transform .12s ease; }
  .send-btn:active { transform: scale(0.9); }

  /* ── PROFILE ── */
  .profile-head { display: flex; flex-direction: column; align-items: center; padding: 22px 16px 14px; text-align: center; }
  .profile-avatar { width: 76px; height: 76px; border-radius: 999px; background: var(--sea-light); display: flex; align-items: center; justify-content: center; font-size: 34px; margin-bottom: 10px; overflow: hidden; box-shadow: 0 6px 18px -8px rgba(15,28,40,0.25); }
  .profile-name { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 17px; letter-spacing: -0.02em; }
  .profile-bio { font-size: 12.5px; color: var(--muted); margin-top: 4px; max-width: 260px; line-height: 1.5; }
  .stat-row { display: flex; gap: 20px; margin-top: 13px; }
  .stat-item { text-align: center; }
  .stat-num { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 15px; letter-spacing: -0.01em; }
  .stat-lbl { font-size: 10.5px; color: var(--muted); margin-top: 2px; }
  .karma-id-box { margin-top: 11px; background: var(--sea-light); color: var(--sea-dark); border-radius: 999px; padding: 5px 14px; font-size: 12px; }
  .karma-id-box b { font-family: 'Space Grotesk', sans-serif; }

  /* ── PROFILE CONTENT GRID (Instagram-style) ── */
  .profile-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 2px; margin-top: 18px; }
  .profile-grid-tile { position: relative; aspect-ratio: 1 / 1; overflow: hidden; background: var(--sea-light); cursor: pointer; transition: transform .15s ease; }
  .profile-grid-tile:active { transform: scale(0.97); }
  .profile-grid-tile img { width: 100%; height: 100%; object-fit: cover; display: block; }
  .profile-grid-video-icon { position: absolute; top: 6px; right: 6px; color: #fff; filter: drop-shadow(0 1px 3px rgba(0,0,0,0.6)); }

  /* ── MY LISTINGS GRID ── */
  .listings-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-top: 4px; }
  .listings-grid-tile { display: flex; flex-direction: column; cursor: pointer; transition: transform .15s ease; }
  .listings-grid-tile:active { transform: scale(0.97); }
  .listings-grid-thumb { position: relative; aspect-ratio: 1 / 1; border-radius: 16px; overflow: hidden; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 14px -8px rgba(15,28,40,0.18); }
  .listings-grid-thumb img { width: 100%; height: 100%; object-fit: cover; display: block; }
  .listings-grid-emoji { font-size: 28px; }
  .listings-grid-video-badge { position: absolute; top: 6px; right: 6px; color: #fff; filter: drop-shadow(0 1px 3px rgba(0,0,0,0.6)); }
  .listings-grid-boost-badge { position: absolute; top: 6px; left: 6px; font-size: 13px; background: rgba(255,255,255,0.9); border-radius: 999px; width: 22px; height: 22px; display: flex; align-items: center; justify-content: center; }
  .listings-grid-title { font-size: 12px; font-weight: 600; margin-top: 6px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .listings-grid-pm { display: flex; align-items: center; gap: 3px; font-size: 11px; color: var(--sea-dark); margin-top: 2px; }
  .listings-grid-boost-pill { display: flex; align-items: center; justify-content: center; gap: 3px; font-size: 10px; font-weight: 600; padding: 4px 0; margin-top: 5px; border-radius: 999px; border: 1px solid var(--line); background: var(--bg); color: var(--muted); }
  .listings-grid-boost-pill.on { background: var(--sea); color: #fff; border-color: var(--sea); }
  .listings-grid-delete-btn { display: flex; align-items: center; justify-content: center; width: 28px; flex-shrink: 0; border-radius: 999px; border: 1px solid var(--line); background: var(--bg); color: #dc2626; }
  ...
  /* ── PM SPACE (public seller profile) ── */
  .pms-cover { position: relative; height: 120px; background: linear-gradient(135deg, var(--sea), var(--pink)); }
  .pms-cover .back-btn { position: absolute; top: 14px; left: 14px; }
  .pms-avatar-wrap { display: flex; flex-direction: column; align-items: center; margin-top: -44px; padding: 0 16px 16px; text-align: center; }
  .pms-avatar { width: 84px; height: 84px; border-radius: 999px; background: var(--sea-light); border: 4px solid var(--bg); display: flex; align-items: center; justify-content: center; font-size: 38px; }
  .pms-name-row { display: flex; align-items: center; gap: 5px; margin-top: 10px; font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 17px; }
  .pms-username { font-size: 12.5px; color: var(--muted); margin-top: 2px; }
  .pms-city-pill { display: inline-flex; align-items: center; gap: 4px; margin-top: 8px; background: var(--sea-light); color: var(--sea-dark); font-size: 11.5px; font-weight: 600; padding: 5px 12px; border-radius: 999px; }
  .pms-stat-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 0; margin: 18px 16px 4px; border: 1px solid var(--line); border-radius: 16px; overflow: hidden; }
  .pms-stat { text-align: center; padding: 12px 4px; border-right: 1px solid var(--line); }
  .pms-stat:last-child { border-right: none; }
  .pms-stat-num { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 16px; }
  .pms-stat-lbl { font-size: 10.5px; color: var(--muted); margin-top: 2px; }
  .pms-action-row { display: flex; gap: 10px; padding: 16px; }
  .pms-id-box { margin: 0 16px 16px; background: var(--sea-light); color: var(--sea-dark); border-radius: 999px; padding: 6px 16px; font-size: 12.5px; text-align: center; }
  .pms-id-box b { font-family: 'Space Grotesk', sans-serif; }

  /* ── LISTINGS / SAVED ── */
  .listing-card { display: flex; gap: 12px; border: 1px solid var(--line); border-radius: 18px; padding: 12px; margin-bottom: 10px; align-items: center; box-shadow: 0 3px 12px -8px rgba(15,28,40,0.12); transition: transform .15s ease; }
  .listing-card:active { transform: scale(0.98); }
  .listing-thumb { width: 60px; height: 60px; border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 22px; flex-shrink: 0; }
