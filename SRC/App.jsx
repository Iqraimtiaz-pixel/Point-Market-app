...
class SectionErrorBoundary extends React.Component {
  ...
  componentDidCatch(err) { console.warn("Section render error:", err?.message || err); }
  render() {
    if (this.state.hasError) {
      return <div className="empty-state">Something didn't load here — the rest of your profile is unaffected.</div>;
    }
...
function ProfileScreen({ onNavigate, userLocation, currentUser, onOpenDetail, onProfileUpdate }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q,
      ...
      (err) => {
        // Previously this only logged a warning and left myPosts as [],
        // which renders identically to "you haven't posted anything yet" —
        // masking real failures like a missing Firestore composite index.
  ...
  return (
    <div className="kt-scroll">
        ...
        <div className="profile-avatar">
          {displayAvatarUrl ? (
            <SectionErrorBoundary>
              <img src={displayAvatarUrl} alt="" className="profile-avatar-img" onError={(e) => { e.currentTarget.style.display = "none"; }} />
            </SectionErrorBoundary>
          ) : displayAvatar}
        </div>
          ...
          {myPostsError ? (
            ...
            return (
              <div key={post.id} className="profile-grid-tile" onClick={() => onOpenDetail && onOpenDetail(post)}>
                {tileSrc && <img src={tileSrc} alt="" onError={(e) => { e.currentTarget.style.display = "none"; }} />}
      ...
      {showEdit && (
        <SectionErrorBoundary>
          <EditProfileSheet onClose={() => setShowEdit(false)} currentUser={currentUser} onProfileUpdate={onProfileUpdate} />
        </SectionErrorBoundary>
      )}
    </div>
  );
