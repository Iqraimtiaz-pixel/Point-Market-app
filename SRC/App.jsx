/* ============================================================
   POINT MAKER (PM) â€” Production App
   Auth flow (splash â†’ signup â†’ otp â†’ security â†’ success)
...
// â”€â”€ Firebase (real ES module imports â€” no window globals) â”€â”€
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   ROOT â€” switches between Auth flow, Main App, and Admin Dashboard
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   FIREBASE AUTH â€” real Firebase Auth + Firestore SDK,
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   ðŸ’° LIVE WALLET BALANCE â€” always reads pmPoints straight from Firestore.
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   ðŸ‘¤ LIVE PROFILE STATS â€” karma, trades, followers, following.
...
function getFriendlyAuthError(error, fallback) {
  ...
  if (code === "auth/operation-not-allowed") {
    return "This sign-in method is disabled in Firebase Authentication. Enable Phone and Google in Firebase Console â†’ Authentication â†’ Sign-in method.";
  }

  if (code === "auth/unauthorized-domain") {
    return "This domain is not authorized for Firebase Authentication. Add localhost or your deployed domain under Authentication â†’ Settings â†’ Authorized domains.";
  }
  ...
  if (code === "auth/network-request-failed") {
    return "Network error â€” check your internet connection and try again.";
  }
...
function AuthFlow({ onComplete, onAdminMode }) {
  ...
  // One-shot guard â€” prevents double-completion when both onAuthStateChanged AND
  ...
  // â”€â”€ Auth loading guard: if Firebase hasn't resolved the session in 6 s, go to splash â”€â”€
  useEffect(() => {
  ...
  // â”€â”€ Auto-login: Firebase Auth persists session automatically â”€â”€
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        // Returning user â€” fetch their Firestore profile
        try {
  ...
  // â”€â”€ Step 1: Send OTP via Firebase Phone Auth â”€â”€
  const sendOtp = async () => {
    setAuthError("");
    try {
  ...
  // â”€â”€ Step 3: Create Firestore profile (new users only) â”€â”€
  const createProfile = async (user, location) => {
    try {
      ...
      // Profile may already exist (returning user via new device) â€” not an error
    }
  };

  // â”€â”€ Google Sign-In â€” shared completion logic for both popup and redirect flows â”€â”€
  const completeGoogleUser = async (user) => {
    try {
      const profile = await getUserProfile(user.uid);
      if (profile) {
        // Returning Google user â€” load existing profile
        ...
        // First-time Google user â€” create their Firestore profile
        await createUserProfile(user.uid, {
          phone:             user.phoneNumber || "",
          email:             user.email       || "",
      ...
      // Firestore error â€” still complete auth with what we have
      safeComplete(
        { uid: user.uid, email: user.email, fullName: user.displayName || "" },
        DEFAULT_LOCATION
      );
  ...
  const signInWithGoogle = async () => {
    ...
    try {
      ...
      // Popup blocked or unsupported (common in embedded/mobile webviews) â€” fall
  ...
  const handleSignupComplete = async (location) => {
    ...
    if (formData.email && formData.password && formData.password.length >= 6) {
      try {
        ...
        // Non-fatal â€” email may already belong to another account; skip silently
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   LOGIN â€” phone number entry (for returning users)
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  ...
  return (
    <div className="kt-scroll">
      <div className="screen-pad" style={{ paddingTop: 18 }}>
        ...
        >
          {loading
            ? (mode === "phone" ? "Sending codeâ€¦" : "Signing inâ€¦")
            : mode === "phone"
...
function SplashScreen({ onNext, onLogin, onAdminMode, onGoogleSignIn, error }) {
  ...
  return (
    <div className="splash">
      ...
      >
        {/* The logo image already contains the "PointMarket Â· TRADE WITH POINTS" wordmark */}
        <PmHexLogo size="splash" />
      </div>
      <p className="tagline">Where <b>good deals happen</b>.<br />Trade skills, items &amp; services â€” earn PM Points with every honest trade.</p>
  ...
  return (
    <div className="kt-scroll">
      ...
      <div className="screen-pad">
        ...
        <div className="eyebrow"><Shield size={13} /> Step 1 of 5 Â· Create Account</div>
        ...
        <FieldWrap label="Email address (optional â€” enables email login)" icon={<Mail size={15} />} error={errors.email}>
        ...
        <button className="kt-btn primary" disabled={loading} style={loading ? { opacity: 0.6 } : {}} onClick={next}>
          {loading ? "Sending verification codeâ€¦" : <><Smartphone size={15} /> Send verification code</>}
        </button>
      </div>
    </div>
  );
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   GOOGLE ICON â€” used in the Google sign-in button
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
function GoogleIcon() {
  return (
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   DOB PICKER â€” modern 3-dropdown date selector
...
function DobPicker({ value, onChange }) {
  ...
  // â”€â”€ Local state for each field â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // in useState â€” it is NOT derived from the value prop on every render, so
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   SCREEN 3 â€” OTP VERIFICATION (real Firebase code)
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
function OtpScreen({ phone, onVerified, onResend, onBack, error: externalError }) {
  ...
  return (
    <div className="kt-scroll">
      ...
      <div className="screen-pad">
        <button type="button" className="back-btn-inline" onClick={() => onBack && onBack()} style={{ marginBottom: 16 }}><ChevronLeft size={20} style={{ pointerEvents: "none" }} /></button>
        <div className="eyebrow"><Smartphone size={13} /> Step 2 of 5 Â· Phone Verification</div>
        ...
        <div className="resend-row">
          {resendCooldown > 0
            ...
            : <>Didn't get a code? <span className="resend-link" onClick={resend}>{resending ? "Sendingâ€¦" : "Resend code"}</span></>
          }
        </div>
        ...
        <button
          ...
          onClick={verify}
        >
          {loading ? "Verifyingâ€¦" : <>Verify &amp; continue <ChevronRight size={16} /></>}
        </button>
      </div>
    </div>
  );
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   ðŸ“ LOCATION-BASED SMART MATCHING â€” DATA LAYER
...
// Returns a small randomized offset (~0â€“6km) around a city center,
...
function jitterCoord(lat, lng, seed) {
  ...
  const dLat = (r - 0.5) * 0.09;  // â‰ˆ Â±5km
  const dLng = (r2 - 0.5) * 0.09;
  return { lat: lat + dLat, lng: lng + dLng };
}

// Haversine formula â€” great-circle distance between two GPS points, in KM
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   ðŸ¤– AI AUTHENTICITY & KARMA SCORING ENGINE
   â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// Core engine â€” accepts the raw listing draft, returns a full AI report
function runAiAuthenticityEngine({ title, desc, category, mediaFile, isVideo, aiValue }) {
  ...
  // â”€â”€ 1 & 2 & 3: Content / Video / Image analysis proxies â”€â”€
  ...
  // Title/description â†” media relevance proxy (keyword overlap heuristic)
  ...
  // â”€â”€ Condition score (image-led: damage/wear proxy from description keywords) â”€â”€
  ...
  // â”€â”€ Content quality (production proxy: clarity + completeness of media) â”€â”€
  const contentQuality = clampScore(clarity - manipulationRisk * 0.4 + (hasMedia ? 8 : -25));

  // â”€â”€ Category value index (0-100 normalized market-demand weight) â”€â”€
  const categoryValue = categoryValueFor(category);

  // â”€â”€ Listing completeness â”€â”€
  let completeness = 0;
  ...
  // â”€â”€ Fraud / suspicious-content detection â”€â”€
  const flags = [];
  if (manipulationRisk > 28) flags.push("Possible manipulated or AI-generated media detected");
  if (genericName) flags.push("Generic/stock-style filename â€” possible reused content");
  if (!hasMedia) flags.push("No media uploaded â€” cannot verify authenticity");
  if (descLen < 10) flags.push("Description too short to verify realistic claims");
  if (overlap === 0 && titleLen > 0 && descLen > 0) flags.push("Title and description don't clearly match â€” review for accuracy");
  if (fileSizeMb < 0.05 && hasMedia) flags.push("Media file unusually small â€” possible low-effort or placeholder upload");
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   ðŸ’³ AI PAYMENT VERIFICATION ENGINE â€” JazzCash + PM Points
   â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function runPaymentVerificationEngine({ expectedAmount, screenshotFile, transactionId }) {
  ...
  // â”€â”€ 1. Duplicate screenshot detection â”€â”€
  ...
  // â”€â”€ 2. Duplicate transaction ID detection â”€â”€
  ...
  // â”€â”€ 3. Transaction ID format check (JazzCash TxIDs are typically alphanumeric, 8-16 chars) â”€â”€
  ...
  if (txId.length === 0) flags.push("No transaction ID entered â€” cannot cross-check against JazzCash records");

  // â”€â”€ 4. Screenshot presence & quality proxy (resolution/size as a clarity stand-in) â”€â”€
  if (!hasScreenshot) flags.push("No payment screenshot uploaded â€” cannot verify receipt");
  const genericName = screenshotFile && /^(image|img|screenshot|file|download)[\d_\-]*\.(jpg|png|jpeg)$/i.test(screenshotFile.name || "");
  if (genericName) flags.push("Generic filename detected â€” possible reused or unedited stock screenshot");
  if (hasScreenshot && fileSizeMb < 0.03) flags.push("Screenshot file unusually small â€” may be cropped, edited, or a placeholder image");

  // â”€â”€ 5. Manipulation risk proxy â”€â”€
  let manipulationRisk = rand() * 14 + (genericName ? 20 : 0) + (fileSizeMb < 0.03 && hasScreenshot ? 22 : 0);

  // â”€â”€ 6. Amount-match confidence (simulated OCR readback of the receipt amount) â”€â”€
  ...
  // â”€â”€ 7. Receipt structure validity (JazzCash layout proxy: presence checks) â”€â”€
  ...
  // â”€â”€ 8. Overall verification confidence (weighted composite) â”€â”€
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   ðŸ›¡ï¸ ADMIN DASHBOARD â€” shared state, access control, analytics
   â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   Production note: every number on this dashboard is computed
   live from the data that actually exists in this prototype
   (USER_DIRECTORY, FEED, payment transactions, reports, reviews â€”
   ...
   that isn't connected here â€” those sections are intentionally
   shown as "Not Connected" rather than faked.
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */

const ADMIN_PASSCODE = "PM-ADMIN-2026"; // Change this before deployment â€” use environment variable in production
...
// Seeded pseudo-historical series for charts â€” anchored to a real current total
...
function computePlatformAnalytics({ users, feed, transactions, reports, reviews }) {
  ...
  const pointsCirculating = PM_POINTS_PACKAGES.reduce((sum, p) => sum + p.points, 0) * 3; // rough float estimate from packages Ã— avg purchases this session
...
/*
  â”€â”€â”€ BACKEND SCHEMA (reference) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  DeviceFingerprint {
    id, fingerprint_hash, user_id (fkâ†’User),
  ...
  On fresh signup:
    â†’ INSERT KarmaLedger { amount:100, status:'LOCKED', reason:'WELCOME_BONUS' }
    â†’ Unlocks only after first item/skill video post
  On reused device:
    â†’ block welcome bonus (Sybil flag), allow account creation
*/

/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   ROOT â€” controls which screen shows
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   SCREEN 4 â€” DEVICE FINGERPRINT & SECURITY
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
function SecurityScreen({ fingerprint, deviceBlocked, onDone }) {
  ...
  return (
    <div className="kt-scroll">
      ...
      <div className="screen-pad">
        <div className="eyebrow"><Shield size={13} /> Step 3 of 5 Â· Security Gateway</div>
        <h1 className="screen-h1">Verifying your accountâ€¦</h1>
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   SCREEN 4 â€” CITY + GPS LOCATION SETUP
...
function LocationScreen({ onDone }) {
  ...
  const requestGps = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        // Permission denied â€” fall back to city center so the app still works
        const c = PAKISTANI_CITIES.find((c) => c.name === selectedCity);
        setCoords({ lat: c.lat, lng: c.lng });
        setGpsState("denied");
      },
  ...
  return (
    <div className="kt-scroll">
      ...
      <div className="screen-pad">
        <div className="eyebrow"><MapPin size={13} /> Step 4 of 5 Â· Set Your Location</div>
        ...
        {selectedCity && (
          <div className="gps-card">
            ...
            {gpsState === "requesting" && (
              <div className="gps-loading"><Compass size={18} className="spin" /> Requesting locationâ€¦</div>
            )}
            {gpsState === "granted" && (
            ...
            {gpsState === "denied" && (
              <div className="gps-warning">
                ...
                <div>
                  ...
                  <div className="gps-sub">No problem â€” we'll use {selectedCity}'s city center instead. You can enable precise GPS anytime from Settings.</div>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="hint-row" style={{ marginTop: 18 }}>
          <MapPin size={13} />
          <span>You can update your city or GPS location later anytime from Profile â†’ Settings â†’ Location.</span>
        </div>
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   SCREEN 5 â€” SUCCESS + REWARD LOCK
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
    ...
    <div className="kt-scroll">
      ...
      <div className="screen-pad success-pad">
        ...
        <div className="success-hero">
          <div className="success-avatar">ðŸ§‘â€ðŸš€</div>
        ...
        <div className={`reward-card ${deviceBlocked ? "blocked-card" : ""}`}>
          ...
          {deviceBlocked ? (
            <div className="reward-lock-tag blocked">
              <AlertTriangle size={12} /> Bonus withheld â€” device reuse detected
            </div>
          ) : (
        ...
        <div className="summary-card">
          ...
          <div className="summary-row"><span>Phone</span><b>Verified âœ“</b></div>
          <div className="summary-row"><span>Location</span><b>{userLocation?.city || "Not set"}</b></div>
          <div className="summary-row"><span>Device</span><b style={{ color: deviceBlocked ? "#ef4444" : "#1d4ed8" }}>{deviceBlocked ? "Flagged" : "Trusted âœ“"}</b></div>
...
.auth-root .hex-logo-svg { width: 100%; height: 100%; }
  ...
  /* â”€â”€ OTP ROW SHAKE (when wrong code entered) â”€â”€ */
  .auth-root .otp-row.otp-shake { animation: shake .35s ease; }

  /* â”€â”€ SCREEN CHROME â”€â”€ */
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   ðŸ›¡ï¸ ADMIN GATE â€” separate secure entry, role-gated
...
const FAQ_ITEMS = [
  ...
  { q: "How do I boost my post?",          a: "Go to Shop â†’ Premium Post Boost, upload your video, and pay Rs. 150 via JazzCash. Your payment is verified automatically by AI." },
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   ðŸ”— SHARED PLATFORM STORE
   A lightweight module-level store (with a simple subscribe
   mechanism) so data created in the user-facing app â€” reports,
   reviews, payment transactions â€” is visible live inside the
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   ðŸ›¡ï¸ ADMIN DASHBOARD â€” Point Maker
...
/* Minimal inline SVG line chart â€” no external charting lib needed */
...
function AdminGrowth({ analytics }) {
  ...
  return (
    <div className="admin-section">
      ...
      <div className="admin-chart-card">
        <div className="admin-chart-head"><span>Engagement Trend â€” DAU / WAU / MAU</span></div>
      ...
      <div className="admin-chart-card">
        ...
        <div className="admin-disconnected"><Database size={15} /> Not connected â€” requires a referral tracking backend</div>
      </div>
    </div>
  );
}

/* â”€â”€ 3. USER BEHAVIOR â”€â”€ */
function AdminBehavior() {
  ...
  return (
    <div className="admin-section">
      ...
      <AdminListCard title="Most Popular Profiles (by Karma Score)" rows={topByKarma.map((u) => ({ left: `${u.avatar} ${u.name}`, right: `â­ ${u.karmaScore}` }))} />
      ...
      <div className="admin-chart-card">
        ...
        <div className="admin-disconnected"><Database size={15} /> Not connected â€” requires server-side search query logging</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="admin-section">
      ...
      <div className="admin-stat-grid">
        ...
        <AdminStatCard icon={ThumbsUp}   label="Positive Reviews" value={analytics.positiveReviews} sub="4â˜… and above" color="#16a34a" />
        <AdminStatCard icon={ThumbsDown} label="Negative Reviews" value={analytics.negativeReviews} sub="2â˜… and below" color="#dc2626" />
      </div>
              ...
              <div key={r.id} className="admin-review-row">
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <span style={{ fontSize: 11, color: "#eab308" }}>{"â˜…".repeat(r.rating)}{"â˜†".repeat(5 - r.rating)}</span>
                  </div>
  ...
  return (
    <div className="admin-section">
      ...
      <div className="admin-chart-card">
        ...
        {open.length === 0 ? (
          <div className="admin-empty">No open reports. Nice and clean ðŸŽ‰</div>
        ) : (
          <div className="admin-list">
            {open.map((r) => (
              <div key={r.id} className="admin-report-row">
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 10.5, color: "#9aa3af", marginTop: 4 }}>From {r.reporter} Â· {new Date(r.at).toLocaleString()}</div>
                </div>
            ...
            return (
              <div key={item.id} className="admin-mod-row">
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#6b7587" }}>{item.user} Â· {item.city} Â· {item.contentType}</div>
                </div>
                {removed ? (
            ...
            return (
              <div key={u.id} className="admin-mod-row">
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#6b7587" }}>{u.trades} trades Â· â­ {u.karmaScore}</div>
                </div>
                {suspended ? (
...
/* â”€â”€ 7. PERFORMANCE MONITORING â€” honestly marked, not faked â”€â”€ */
    ...
    <div className="admin-section">
      ...
      {[
        ...
        <div key={label} className="admin-chart-card">
          ...
          <div className="admin-disconnected"><Icon size={15} /> Not connected â€” requires a live backend / hosting environment</div>
        </div>
      ))}
      ...
      {["Firebase Analytics", "Firebase Crashlytics", "Firebase Performance Monitoring", "Firebase Authentication Tracking", "Push Notification Analytics"].map((label) => (
        <div key={label} className="admin-chart-card">
          <div className="admin-chart-head"><span>{label}</span></div>
          <div className="admin-disconnected"><Loader size={15} /> Not connected â€” connect a Firebase project to enable</div>
        </div>
      ))}
    </div>
  );
...
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   ADMIN_CSS â€” self-contained enterprise dashboard styles
...
function MainApp({ initialLocation, currentUser, onLogout, onProfileUpdate }) {
  ...
  // â”€â”€ Auth guard: if Firebase revokes the token mid-session, force logout â”€â”€
  useEffect(() => {
  ...
  // â”€â”€ Follow system: in-memory set of usernames the current user follows â”€â”€
  const [followedUsers, setFollowedUsers] = useState(new Set());
  const [activeProfileUser, setActiveProfileUser] = useState(null); // username being viewed in PM Space

  // â”€â”€ Dark mode: load saved preference, persist on change (real localStorage) â”€â”€
  useEffect(() => {
    try {
      const saved = window.localStorage.getItem("pm-dark-mode");
      if (saved === "true") setDarkMode(true);
    } catch (e) { /* localStorage unavailable (private browsing) â€” default to light */ }
  }, []);
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   TOP APP BAR â€” PM logo Â· search Â· notifications Â· profile
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  ...
  return (
    <div className="topbar">
      ...
      <div className="search-pill">
        <Search size={15} />
        <input className="search-input" placeholder="Search items, skills, tradersâ€¦" value={query} onChange={(e) => onQueryChange(e.target.value)} />
      </div>
      {rightExtra}
...
function HomeScreen({ userLocation, currentUser, onOpenDetail, onTrade, onOpenMap, onOpenProfile, onOpenPmSpace, onOpenCreate }) {
  ...
  // â”€â”€ Real Firestore posts feed â”€â”€
  ...
  useEffect(() => {
    ...
    try {
      // Listen to posts collection â€” ordered by createdAt descending, boosted first
      ...
      const unsub = onSnapshot(q,
        (snap) => {
          ...
          // Only show real Firestore posts â€” no static merge
  ...
  }, []); // Run once on mount â€” onSnapshot keeps it live automatically
      ...
      <div className="kt-scroll">
        ...
        {feedLoading && (
          <div className="feed-loading">
            ...
            <span>Loading nearby listingsâ€¦</span>
          </div>
        )}
              ...
              {nearbyForYou.map((item) => (
                <div key={item.id} className="nearby-card" onClick={() => onOpenDetail(item)}>
                  {item.thumbnailUrl ? (
                    ...
                    <div className="nearby-thumb" style={{ background: `linear-gradient(160deg, ${item.color1 || "#bdeede"}, ${item.color2 || "#8fd9bd"})` }}>ðŸŽ¥</div>
                  )}
                  <div className="nearby-title">{item.title}</div>
                  <div className="nearby-meta">
                    <span className="nearby-dist">{item.distanceKm <= 2 ? "ðŸ”¥" : "ðŸ“"} {formatDistance(item.distanceKm)}</span>
        ...
        {/* Feed error state â€” only shown when Firestore actually failed */}
        ...
        {/* Main feed â€” professional empty state when there simply are no listings yet */}
        ...
        {!feedLoading && demoItems.length > 0 && (
          <div className="demo-section">
            <div className="demo-header">Explore Demos â€” Browse Only</div>
            {demoItems.map((d) => (
  ...
  enriched
    ...
    .forEach((i) => {
      notifs.push({
        id: `match-${i.id}`,
        icon: "ðŸ”¥",
        text: `Someone ${formatDistance(i.distanceKm)} away is looking for a ${i.needsItem || "trade"} â€” your nearby match: "${i.title}"`,
        item: i,
      });
    });
  // Cross-match example: bicycle â†” study table (Intelligent Requirement Matching demo)
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   FEED CARD â€” fixed action sidebar
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
function FeedCard({ item, onOpenDetail, onTrade, onOpenPmSpace }) {
  ...
  // â”€â”€ Cloudinary URL helpers (inline â€” no external import needed in single file) â”€â”€
    ...
    <div className="feed-card">
      ...
      >
          ...
          <>
            ...
            {item.videoDuration ? (
              <span className="duration-badge">
                â–¸ {Math.floor(item.videoDuration / 60)}:{String(Math.round(item.videoDuration % 60)).padStart(2, "0")}
              </span>
            ) : (
              <span className="duration-badge">â–¸ Video</span>
            )}
          </>
        )}

        <span className="category-badge">{item.category || "Video"}</span>
        {item.isDemo && <span className="demo-badge" title="Demo listing â€” browse only">EXPLORE DEMO</span>}
        ...
        <div className="feed-actions">
          ...
          <div className="act-wrap">
            <button className="act-btn" onClick={(e) => { e.stopPropagation(); if (item.isDemo) { setToast("Demo listing â€” browse only"); setTimeout(() => setToast(null), 1500); return; } setShowComments(true); }} title={item.isDemo ? "Demo listing â€” comments disabled" : "Comments"}>
          ...
          <div className="act-wrap">
            <button className="act-btn trade-btn" onClick={(e) => { e.stopPropagation(); if (item.isDemo) { setToast("Demo listing â€” trade disabled"); setTimeout(() => setToast(null), 1500); return; } onTrade(item); }} style={item.isDemo ? { opacity: 0.45, cursor: "not-allowed" } : {}} title={item.isDemo ? "Demo listing â€” trade disabled" : "Trade"}>
              <Handshake size={18} />
            </button>
        ...
        {/* Click overlay â€” tapping non-play area opens detail */}
        {!playing && (
          <div
      ...
      <div className="feed-body">
        ...
        {item.distanceKm !== undefined && (
          <div className="loc-badge-row">
            ...
            <span className="loc-badge dist">{item.distanceKm <= 2 ? "ðŸ”¥" : "ðŸ“"} {formatDistance(item.distanceKm)} Away</span>
            <span className="loc-badge karma">â­ Karma Score: {Math.round(item.karmaScore * 19.2)}</span>
          </div>
        )}

        <div className="ai-row"><Sparkles size={13} /> AI Value: {item.aiValue?.toLocaleString() || item.recommendedPm?.toLocaleString() || "â€”"} PM</div>
      </div>
...
function CommentsSheet({ item, onClose }) {
  ...
  const post = () => {
    ...
    setComments([...comments, { user: "@you.trades", avatar: "ðŸ§‘â€ðŸš€", text: input }]);
    setInput("");
  };
  return (
    <div className="sheet-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="sheet">
        ...
        <div className="chat-input-row">
          <input className="chat-input" placeholder="Add a commentâ€¦" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && post()} />
      ...
      <div className="sheet">
        ...
        {mode === "item" && !sent && (
          <>
            ...
            <p className="sheet-sub">Tell the seller what you want to trade â€” any item or skill is welcome.</p>
...
function DetailScreen({ item, userLocation, onBack, onTrade, onAiHub, onBattle, onOpenPmSpace }) {
  ...
  // â”€â”€ Cloudinary media resolution â”€â”€
  const CLOUD      = "dzhy4zx5g";
  const isVideo    = item.mediaType === "video";
  const hasMedia   = !!(item.mediaUrl || item.thumbnailUrl || item.publicId);
  const thumbSrc   = item.thumbnailUrl
  ...
  const submitRating = (stars) => {
    ...
    submitReview({ reviewer: "@you.trades", avatar: "ðŸ§‘â€ðŸš€", rating: stars, text: "", target: item.user });
    setRatingSubmitted(true);
  };
  ...
  return (
    <div className="kt-scroll">
        ...
        {playing && item.mediaUrl ? (
          ...
          <>
            ...
            {thumbSrc && !thumbError ? (
              ...
              <>
                <div className="video-pattern" />
                <span style={{ fontSize: 70, position: "relative", zIndex: 1 }}>ðŸŽ¥</span>
              </>
            )}
            ...
            {isVideo && item.mediaUrl && (
              ...
              >
                <div className="play-circle" style={{ width: 56, height: 56, fontSize: 22 }}>â–¶</div>
              </div>
            )}
      ...
      <div style={{ padding: "14px 16px 0" }}>
        ...
        {distanceKm !== null && (
          <div className="loc-badge-row" style={{ marginBottom: 4 }}>
            ...
            <span className="loc-badge dist">{distanceKm <= 2 ? "ðŸ”¥" : "ðŸ“"} {formatDistance(distanceKm)} Away</span>
      ...
      <div className="info-grid">
        ...
        <div className="info-box"><div className="info-label">Location</div><div className="info-value">{item.city || "â€”"}</div></div>
      ...
      <div className="detail-actions">
        <button className="kt-btn" onClick={() => { if (item.isDemo) return; onTrade(); }} style={item.isDemo ? { opacity: 0.45, cursor: "not-allowed" } : {}} title={item.isDemo ? "Demo listing â€” trade disabled" : "Trade"}><Handshake size={16} /> {item.isDemo ? "Demo" : "Trade"}</button>
        <button className="kt-btn ghost" onClick={() => { if (item.isDemo) return; onBattle(); }} style={item.isDemo ? { opacity: 0.45, cursor: "not-allowed" } : {}} title={item.isDemo ? "Demo listing â€” battle disabled" : "Battle"}><TrendingUp size={16} /> {item.isDemo ? "Demo" : "Battle"}</button>
      </div>
...
function AiHubScreen({ onBack }) {
  return (
    <div className="kt-scroll">
      ...
      <div className="ai-banner"><Sparkles size={20} /><div><div className="ai-banner-title">Item unavailable</div><div className="ai-banner-sub">Searched: "iPhone 14" â€” here's what AI found.</div></div></div>
...
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   SHOP â€” PM Points purchase, Boost, Certification
  ...
  return (
    <div className="kt-scroll">
      ...
      <div className="boost-hero">
        ...
        <div className="boost-hero-sub">Get seen first. Boosted listings appear at the top of the Home Feed and rank higher in search â€” pay once via JazzCash, no subscription.</div>
      </div>
      ...
      {/* â”€â”€ SELLER BADGES INFO CARD â”€â”€ */}
          ...
          {transactions.map((tx) => (
            <div key={tx.id} className="payment-history-row">
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11.5, color: "#6b7587" }}>Rs. {tx.amount.toLocaleString()} Â· {new Date(tx.at).toLocaleString()}</div>
              </div>
      ...
      {/* Native ad slot â€” hidden while the payment overlay is open */}
...
/* â”€â”€ JazzCash brand mark (clean SVG badge â€” not the literal trademarked logo file) â”€â”€ */
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   JAZZCASH PAYMENT FLOW â€” package/boost/cert â†’ instructions â†’
   screenshot + TxID upload â†’ AI verification â†’ result
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
          ...
          <>
            ...
            {!screenshotUrl ? (
              <div className="upload-box" style={{ margin: "0 0 14px", cursor: "pointer" }} onClick={() => fileRef.current?.click()}>
                ...
                <div style={{ fontSize: 12, marginTop: 4, color: "#6b7587" }}>Choose from gallery â€” must show amount, date/time, and Transaction ID</div>
              </div>
            ) : (
...
function PaymentVerifyingState() {
  ...
  const steps = ["Reading receipt detailsâ€¦", "Matching payment amountâ€¦", "Checking for duplicatesâ€¦", "Confirming JazzCash receipt formatâ€¦"];
  useEffect(() => {
  ...
  return (
    <div className="ai-scan-wrap" style={{ padding: "40px 16px 24px" }}>
      ...
      <div className="ai-scan-sub">Analyzing your JazzCash receiptâ€¦</div>
  ...
  return (
    <>
      <div className="ai-result-hero" style={{ paddingTop: 8 }}>
        ...
        <div className="ai-result-item-title">{label} Â· Rs. {amount.toLocaleString()}</div>
      </div>
      ...
      {report.status === "review" && (
        <div className="warning-box" style={{ margin: "0 16px 14px", background: "rgba(184,134,11,0.08)", borderColor: "rgba(184,134,11,0.25)", color: "#92660a" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, marginTop: 3 }}>We couldn't confirm every detail automatically. Your payment is queued for review and points will be credited once confirmed â€” usually within a few hours.</div>
          </div>
        </div>
      )}
      ...
      <div className="kp-result-card">
        ...
        <div className="kp-result-formula">Transaction ID: {report.transactionId || "â€”"}</div>
      </div>
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   CLOUDINARY UPLOAD â€” handled via services/cloudinaryService.js
    ...
    <div className="cld-progress-wrap">
      <div className="cld-progress-top">
        <span className="cld-progress-label">
          {progress < 100 ? `Uploadingâ€¦ ${progress}%` : "Processingâ€¦"}
        </span>
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   BOOST UPLOAD â€” wired to Cloudinary
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
function BoostUpload({ onUploaded }) {
  ...
  return (
    <>
      ...
      >
        {uploading ? "Uploading to Cloudinaryâ€¦" : <>Continue <ChevronRight size={16} /></>}
      </button>
    </>
  );
}
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   CREATE POST â€” full Cloudinary + AI + Firestore flow
   Stage machine:
     form â†’ uploading â†’ analyzing â†’ results â†’ saving â†’ posted
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  ...
  return (
    <div className="kt-scroll">
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10 }}>
            <div>
              ...
              <span style={{ fontSize: 11, color: "#6f8b80", display: "block", marginTop: 2 }}>
                {(mediaFile?.size / 1024 / 1024).toFixed(1)} MB Â· {isVideo ? "Video" : "Image"}
              </span>
            </div>
            <button className="kt-btn ghost" style={{ width: "auto", padding: "7px 12px", fontSize: 12 }} onClick={() => { setMediaUrl(null); setMediaFile(null); }}>Replace</button>
          </div>
      ...
      <div className="field-block">
        ...
        <input className="field-input" placeholder="e.g. Study Table, Headphones, Bicycleâ€¦" value={needsItem} onChange={(e) => setNeedsItem(e.target.value)} />
      </div>
...
/* â”€â”€ AI Engine: Analyzing (loading) screen â”€â”€ */
function AiAnalyzingScreen({ isVideo }) {
  const [stepIdx, setStepIdx] = useState(0);
  const steps = isVideo
    ? ["Scanning video qualityâ€¦", "Checking for manipulationâ€¦", "Matching content to listingâ€¦", "Calculating Karma Scoreâ€¦"]
    : ["Scanning image clarityâ€¦", "Detecting condition & wearâ€¦", "Verifying authenticityâ€¦", "Calculating Karma Scoreâ€¦"];
  ...
  return (
    <div className="kt-scroll">
      ...
      <div className="ai-scan-wrap">
        ...
        <div className="ai-scan-sub">Analyzing your {isVideo ? "video" : "image"} listingâ€¦</div>
        <div className="ai-scan-steps">
          {steps.map((s, i) => (
...
/* â”€â”€ AI Engine: Results screen â”€â”€ */
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   INBOX / CHAT LIST â€” with Find User
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
const CHATS = [];
        ...
        {result && (
          <div className="user-result">
            ...
            <div style={{ flex: 1, cursor: "pointer" }} onClick={() => goToProfile(result.user)}>
              ...
              <div style={{ fontSize: 12.5, color: "#6f8b80" }}>{result.user} Â· ID: {result.id}</div>
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      {selected ? (
        <div className="map-detail-card">
          <div className="map-detail-thumb" style={{ background: `linear-gradient(160deg, ${selected.color1}, ${selected.color2})` }}>ðŸŽ¥</div>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div className="loc-badge-row" style={{ marginTop: 4 }}>
              <span className="loc-badge dist">{selected.distanceKm <= 2 ? "ðŸ”¥" : "ðŸ“"} {formatDistance(selected.distanceKm)}</span>
              <span className="loc-badge karma">â­ {selected.karmaScore}</span>
            </div>
          </div>
        ...
        <div className="map-list">
          {nearby.slice(0, 4).map((item) => (
            <div key={item.id} className="map-list-row" onClick={() => setSelected(item)}>
              <div className="map-list-thumb" style={{ background: `linear-gradient(160deg, ${item.color1}, ${item.color2})` }}>ðŸŽ¥</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: 12.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.title}</div>
                <div style={{ fontSize: 11, color: "#6f8b80" }}>{item.distanceKm <= 2 ? "ðŸ”¥ Nearby Trader" : "ðŸ“"} Â· {formatDistance(item.distanceKm)} away</div>
              </div>
              <ChevronRight size={15} color="#6f8b80" />
            </div>
          ))}
...
function ProfileScreen({ onNavigate, userLocation, currentUser }) {
  ...
  const displayBio    = currentUser?.bio         || "Trading my way across town, one fair deal at a time ðŸ¤";
  const displayAvatar = currentUser?.avatarEmoji || "ðŸ§‘â€ðŸš€";
  const displayId     = currentUser?.uid ? `PM-${currentUser.uid.slice(0, 6).toUpperCase()}` : "PM-000";
  // â”€â”€ Live profile stats â€” karma, trades, followers, following â€” always
  // from Firestore, defaulting to 0 for brand-new users. Never hardcoded. â”€â”€
  const { karmaScore: karma, totalTrades: trades, followers, following } = useLiveProfileStats(currentUser?.uid);

  // â”€â”€ Live PM Points balance â€” always read from Firestore, never hardcoded â”€â”€
  const { points: pmPoints } = useLivePmPoints(currentUser?.uid);
  const walletSub = pmPoints === null ? "Loadingâ€¦" : `${pmPoints.toLocaleString()} PM`;
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   WALLET â€” real Firestore-backed PM Points balance
   Balance always comes live from users/{uid}.pmPoints â€” never hardcoded.
  ...
  return (
    <div className="kt-scroll">
      ...
      <div className="wallet-balance-card">
        ...
        {loading ? (
          <div className="wallet-balance-amount">Loadingâ€¦</div>
        ) : (
        ...
        <div key={tx.id} className="payment-history-row" style={{ margin: "0 16px" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11.5, color: "#6b7587" }}>
              {tx.createdAt?.toDate ? tx.createdAt.toDate().toLocaleString() : "Just now"} Â· {tx.status === "locked" ? "Locked" : "Available"}
            </div>
          </div>
          <span style={{ fontWeight: 700, fontSize: 13.5, color: tx.type === "debit" ? "#dc2626" : "#16a34a" }}>
            {tx.type === "debit" ? "âˆ’" : "+"}{tx.amount} PM
          </span>
        </div>
      ))}
...
function ListingsScreen({ onBack, currentUser }) {
  ...
  // â”€â”€ Live Firestore subscription â€” this user's own posts, newest first.
  // Always the real source of truth; updates automatically the instant a
  // new listing is created (no manual refresh needed). â”€â”€
  useEffect(() => {
  ...
  // â”€â”€ Category counts â€” Products / Skills / Services / Videos â€” always
  // derived live from the same Firestore data, 0 when nothing exists yet. â”€â”€
    ...
    <div className="kt-scroll">
      ...
      <div style={{ padding: "0 16px 4px" }}>
        <div className="karma-id-box">
          ðŸ“¦ {productsCount} Products &nbsp;Â·&nbsp; ðŸ› ï¸ {skillsCount} Skills &nbsp;Â·&nbsp; ðŸ§° {servicesCount} Services &nbsp;Â·&nbsp; ðŸŽ¥ {videosCount} Videos
        </div>
      </div>
      <div style={{ padding: "12px 16px 16px" }}>
        {loading && (
          <div className="feed-loading"><div className="feed-loading-spinner" /><span>Loading your listingsâ€¦</span></div>
        )}
          ...
          <div key={l.id} className="listing-card">
            ...
            <div className="listing-info">
              ...
              <button className={`boost-btn ${l.isBoosted ? "on" : ""}`} onClick={() => toggleBoost(l.id)}>
                <TrendingUp size={13} /> {l.isBoosted ? "Boosted Â· 24h left" : "Boost this post"}
              </button>
            </div>
          </div>
        ))}
        ...
        {FEED.length === 0 ? (
          ...
          FEED.map((item) => (
            <div key={item.id} className="listing-card" style={{ cursor: "pointer" }} onClick={() => onOpenDetail(item)}>
              <div className="listing-thumb" style={{ background: `linear-gradient(160deg, ${item.color1}, ${item.color2})` }}>ðŸŽ¥</div>
              <div className="listing-info">
    ...
    <div className="kt-scroll">
      ...
      {items.length === 0 ? <div className="empty-state">No orders here yet.</div> : items.map((o, i) => (
        <div key={i} className="order-card">
          ...
          <div style={{ fontSize: 12, color: "#6f8b80", marginTop: 4 }}>With {o.with} Â· {o.status}</div>
        </div>
      ))}
    </div>
  );
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   SETTINGS â€” fully functional sheets
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€ Report a Problem â€” feeds straight into the Admin Dashboard â”€â”€ */
      ...
      <div className="sheet">
        ...
        {!sent ? (
          <>
            ...
            <textarea className="field-textarea" style={{ marginBottom: 16 }} placeholder="Describe what happened, including any listing or user involvedâ€¦" value={details} onChange={(e) => setDetails(e.target.value)} />
...
/* Update city / GPS location from Profile â†’ Settings (per spec) */
  ...
  return (
    <div className="sheet-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="sheet">
        ...
        {gpsState === "requesting" && <div className="empty-state" style={{ padding: "0 0 14px" }}>Requesting locationâ€¦</div>}
        ...
        {gpsState === "denied" && <div className="empty-state" style={{ padding: "0 0 14px" }}>GPS denied â€” using {city} city center instead.</div>}
...
function EditProfileSheet({ onClose, currentUser, onProfileUpdate }) {
  ...
  const [bio,  setBio]  = useState(currentUser?.bio || "Trading my way across town, one fair deal at a time ðŸ¤");
  ...
  return (
    <div className="sheet-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="sheet">
        ...
        {!done ? <button className="kt-btn" onClick={save} disabled={saving}><CheckCircle2 size={16} /> {saving ? "Savingâ€¦" : "Save changes"}</button>
...
function PhoneNumberSheet({ onClose, currentUser, onProfileUpdate }) {
  ...
  const verifyCode = async () => {
    ...
    try {
      ...
      // Updates the phone number on the existing signed-in Firebase Auth user â€”
        ...
        {step === "view" && (
          <>
            <div className="confirm-box" style={{ marginBottom: 14 }}>
              ...
              <div className="confirm-row"><span>Status</span><b style={{ color: "#16a34a" }}>Verified âœ“</b></div>
            </div>
        ...
        {step === "edit" && (
          <>
            ...
            <button className="kt-btn" disabled={!newPhone.trim() || sending} style={(!newPhone.trim() || sending) ? { opacity: 0.4 } : {}} onClick={sendCode}>
              {sending ? "Sendingâ€¦" : <>Send code <Smartphone size={15} /></>}
            </button>
          </>
        )}
        ...
        {step === "otp" && (
          <>
            ...
            <div className="resend-row">
              {resendCooldown > 0
                ? <>Didn't get a code? Resend available in {resendCooldown}s</>
                : <>Didn't get a code? <span className="resend-link" onClick={resend}>{sending ? "Sendingâ€¦" : "Resend code"}</span></>
              }
            </div>
            <button className="kt-btn" disabled={otp.length < 6 || verifying} style={(otp.length < 6 || verifying) ? { opacity: 0.4 } : {}} onClick={verifyCode}>
              {verifying ? "Verifyingâ€¦" : <>Verify <CheckCircle2 size={15} /></>}
            </button>
          </>
        )}
...
/* â”€â”€ About Point Maker â€” full info center per spec â”€â”€ */
const ABOUT_SECTIONS = [
  { q: "What is Point Maker?", a: "Point Maker (PM) is a video-first barter and trading marketplace. Instead of using cash, members trade physical items, services, and skills directly with each other â€” or use PM Points as a flexible in-between currency when a direct swap doesn't line up." },
  ...
  { q: "How do I spend points?", a: "Use PM to sweeten a trade offer (e.g. \"my headphones + 200 PM\"), boost a listing to the top of the Home feed for 24 hours, or pay for Verified Pro Seller certification â€” all from the Shop tab." },
  ...
  { q: "Skill trading guide", a: "List a skill the same way you'd list an item â€” record a short demo video, describe what's included (e.g. \"1-hour guitar lesson\"), and set what you're hoping to trade for. Skills are scored for credibility just like products." },
  { q: "How does AI point valuation work?", a: "When you upload a listing, our AI Authenticity & Karma Scoring Engine analyzes authenticity, condition, content quality, category value, and listing completeness to recommend a fair PM Point value â€” so trades stay balanced without manual guesswork." },
...
const ABOUT_FAQ = [
  { q: "Is Point Maker free to use?", a: "Yes â€” browsing, listing, and trading are completely free. Optional paid features like Post Boost and Verified certification are paid securely through JazzCash." },
    ...
    <div className="sheet-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="sheet">
        ...
        <div className="about-version">
          <Info size={13} /> Point Maker (PM) Â· Version 2.4.0
        </div>
      </div>
      ...
      <div className="sheet">
        ...
        {!sent ? (
          <>
            ...
            <textarea className="field-textarea" style={{ marginBottom: 16 }} placeholder="Describe the issue in detailâ€¦" value={message} onChange={(e) => setMessage(e.target.value)} />
...
const CSS = `
  ...
  /* â”€â”€ CLOUDINARY UPLOAD STAGES â”€â”€ */
  ...
  /* â”€â”€ CLOUDINARY PROGRESS BAR â”€â”€ */
  ...
  /* â”€â”€ CLOUDINARY SUCCESS MEDIA INFO â”€â”€ */
  ...
  /* â”€â”€ ðŸ¤– AI AUTHENTICITY & KARMA SCORING ENGINE â”€â”€ */
  ...
  /* â”€â”€ PREMIUM BOOST â€” redesigned cards â”€â”€ */
  ...
  /* Popular tier â€” recommended, blue gradient + glow */
  ...
  /* Gold tier â€” best value, gold gradient */
  ...
  /* â”€â”€ PAYMENT VERIFICATION RESULT â”€â”€ */
  ...
  /* â”€â”€ PHONE INPUT (in settings sheets) â”€â”€ */
  ...
  /* â”€â”€ ðŸ“ LOCATION-BASED SMART MATCHING â”€â”€ */
  ...
  /* â”€â”€ LOCATION SETTINGS (light theme variant for Settings sheet) â”€â”€ */
  ...
  /* â”€â”€ SMART MATCH PREVIEW (Create Post) â”€â”€ */
