...
// ─────────────────────────────────────────────────────────────────────────────
//  1b. PRODUCT COMMUNITY VERIFICATION
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Record one community verification for a Product listing, and — if this is
 * the 5th unique verifier — atomically award the seller's pre-calculated
 * Product Karma exactly once. Products only; never call this for Skills/
 * Services. Not a public wrapper around creditPoints() — the post-doc update
 * (verifiedBy/karmaStatus) and the seller's pmPoints credit must happen in a
 * single transaction together, or a crash between two separate writes could
 * mark a listing "awarded" without actually crediting the wallet (or the
 * reverse). Mirrors creditPoints()'s exact field name (pmPoints) and ledger
 * convention (_writeLedger) — same pattern, just fused into one atomic
 * multi-document operation because this specific case needs it.
 *
 * @param {string} postId
 * @param {string} verifierUid   — the user performing the verification
 * @param {string} sellerUid     — item.userId, the listing owner
 * @param {number} karmaAmount   — the listing's pre-calculated aiValue
 * @returns {object} { success, reason?, verifiedCount, awarded, amount? }
 */
export async function verifyProductListing(postId, verifierUid, sellerUid, karmaAmount) {
  if (!postId || !verifierUid || !sellerUid) {
    return { success: false, reason: "missing_arguments" };
  }
  if (verifierUid === sellerUid) {
    return { success: false, reason: "seller_cannot_verify_own_listing" };
  }
  ...
  await runTransaction(db, async (tx) => {
    ...
    if (!postSnap.exists()) { result = { success: false, reason: "listing_not_found" }; return; }

    const post = postSnap.data();
    const verifiedBy = post.verifiedBy || [];

    if (post.karmaStatus === "awarded") {
      result = { success: false, reason: "already_awarded", verifiedCount: verifiedBy.length };
      return;
    }
    if (verifiedBy.includes(verifierUid)) {
      result = { success: false, reason: "already_verified", verifiedCount: verifiedBy.length };
      return;
    }
    if (verifiedBy.length >= 5) {
      result = { success: false, reason: "verification_full", verifiedCount: verifiedBy.length };
      return;
    }

    const newCount = verifiedBy.length + 1;
    const reachesThreshold = newCount >= 5;

    if (reachesThreshold) {
      const amount = Math.max(0, Math.round(Number(karmaAmount) || 0));
      tx.update(postRef, { verifiedBy: arrayUnion(verifierUid), karmaStatus: "awarded" });
      if (amount > 0) {
        const sellerRef = doc(db, POINTS_COL.USERS, sellerUid);
        const sellerSnap = await tx.get(sellerRef);
        if (sellerSnap.exists()) {
          tx.update(sellerRef, { pmPoints: increment(amount) });
        }
      }
      result = { success: true, verifiedCount: newCount, awarded: true, amount };
    } else {
      tx.update(postRef, { verifiedBy: arrayUnion(verifierUid) });
      result = { success: true, verifiedCount: newCount, awarded: false };
    }
  });
  ...
  if (result?.success && result?.awarded) {
    await _writeLedger({
      ...
      type: TX_TYPE.REWARD,
      status: TX_STATUS.COMPLETED,
      reason: "Product Karma awarded — 5 community verifications completed",
      postId,
    });
  }
