...
import { PmHexLogo } from "../PmHexLogo";
...
// Category is a free-text field at listing-creation time (CreateScreen.jsx
// has no fixed dropdown — just a text input), so there's no enforced
// taxonomy to pull these from. These are just chip labels for browsing;
// MarketplaceScreen matches them the same case-insensitive substring way
// the existing isSkillCategory/isServiceCategory helpers already do, not
// exact equality, since real listing data won't match a fixed list exactly.
export const PRODUCT_CATEGORIES = [
  { label: "Furniture", emoji: "🛋️" }, { label: "Electronics", emoji: "📱" },
  { label: "Fashion", emoji: "👗" }, { label: "Beauty", emoji: "💄" },
  { label: "Home & Kitchen", emoji: "🍳" }, { label: "Books", emoji: "📚" },
  { label: "Mobile Phones", emoji: "📲" }, { label: "Vehicles", emoji: "🚗" },
  { label: "Sports", emoji: "⚽" },
];
export const SKILL_CATEGORIES = [
  { label: "Graphic Design", emoji: "🎨" }, { label: "Web Development", emoji: "💻" },
  { label: "Video Editing", emoji: "🎬" }, { label: "Tutoring", emoji: "🎓" },
  { label: "Photography", emoji: "📷" }, { label: "Writing", emoji: "✍️" },
  { label: "Beauty Services", emoji: "💇" }, { label: "Repair Services", emoji: "🔧" },
  { label: "Marketing", emoji: "📈" },
];


export function HomeScreen({ userLocation, currentUser, onOpenDetail, onTrade, onOpenMap, onOpenProfile, onOpenPmSpace, onOpenCreate, onOpenProducts, onOpenSkills }) {
  ...
  return (
    <div className="home-locked-light">
      {/* ── Brand header: Row 1 = capped logo badge + greeting (never more than
          2 lines). Row 2 = Karma pill on its own line, so it can never
          overlap the greeting at any screen width. The logo badge hard-clips
          to 44×44 regardless of what PmHexLogo's own size prop resolves to —
          this is the fix, not just passing size="home" again. ── */}
      <div className="home-hero">
        <div className="home-hero-row">
          <div className="home-logo-badge"><PmHexLogo size="home" /></div>
          <div className="home-hero-greet">
            <div className="home-hero-hi">Hello, {currentUser?.username || currentUser?.fullName || "User"} 👋</div>
            <div className="home-hero-tag">Buy, Sell, Trade &amp; Learn with Karma Points</div>
          </div>
        </div>
        <div className="home-karma-pill">
          <Sparkles size={13} />
          <span className="home-karma-label">My Karma Points</span>
          <span className="home-karma-amount">{(currentUser?.pmPoints ?? 0).toLocaleString()} KP</span>
        </div>
      </div>
      ...
      {/* ── Products / Skills primary entry points — compact horizontal cards ── */}
      <div className="home-action-grid">
        <div className="home-action-card products" onClick={() => onOpenProducts?.()}>
          <div className="home-action-icon"><ShoppingBag size={18} /></div>
          <div className="home-action-text">
            <div className="home-action-title">Products</div>
            <div className="home-action-cta">Explore <ChevronRight size={12} /></div>
          </div>
        </div>
        <div className="home-action-card skills" onClick={() => onOpenSkills?.()}>
          <div className="home-action-icon"><Handshake size={18} /></div>
          <div className="home-action-text">
            <div className="home-action-title">Skills</div>
            <div className="home-action-cta">Explore <ChevronRight size={12} /></div>
          </div>
        </div>
      </div>
      ...
      <div className="kt-scroll">
        ...
        {/* Explore Categories — Products, browsable by category */}
        {!feedLoading && (
          <div className="cat-section">
            <div className="cat-header">Explore Categories</div>
            <div className="cat-scroll">
              {PRODUCT_CATEGORIES.map((c) => (
                <div key={c.label} className="cat-chip" onClick={() => onOpenProducts?.(c.label)}>
        ...
        {/* Explore Skills — Skills/Services, browsable by category */}
        {!feedLoading && (
          <div className="cat-section">
            <div className="cat-header">Explore Skills</div>
            <div className="cat-scroll">
              {SKILL_CATEGORIES.map((c) => (
                <div key={c.label} className="cat-chip skills" onClick={() => onOpenSkills?.(c.label)}>
                  <span className="cat-chip-emoji">{c.emoji}</span>
                  <span className="cat-chip-label">{c.label}</span>
                </div>
              ))}
