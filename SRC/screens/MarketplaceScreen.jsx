...
export function MarketplaceScreen({ filter, initialCategory, userLocation, currentUser, onBack, onOpenDetail, onOpenNotifs }) {
  ...
  const enriched = categoryFiltered.map((item) => ({
    ...item,
    distanceKm: (typeof item.lat === "number" && typeof item.lng === "number" && typeof userLocation?.lat === "number" && typeof userLocation?.lng === "number")
  ...
  // "Whole City" bypasses distance entirely — the Firestore query above already
  // scopes results to activeCity, so this is a real city match, not a fake
  // "maxKm: Infinity" trick. The 5/10/20 KM filters only ever use real,
  // known coordinates — a listing with no coordinates is excluded from
  // those rather than assigned a made-up distance.
  const distScoped = activeFilter.key === "city"
    ? enriched
    : enriched.filter((item) => item.distanceKm != null && item.distanceKm <= activeFilter.maxKm);
          ...
          <div className="listings-grid market-grid">
            {searchFiltered.map((item) => (
              <div key={item.id} className="listings-grid-tile" onClick={() => openItemDetail(item)}>
                ...
                <div style={{ fontSize: 10, color: "#6f8b80", marginTop: 1 }}>{item.user} · {item.distanceKm != null ? formatDistance(item.distanceKm) : item.city}</div>
              </div>
            ))}
          </div>
        )}
