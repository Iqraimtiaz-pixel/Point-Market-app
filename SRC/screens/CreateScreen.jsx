...
export function CreateScreen({ currentUser, userLocation }) {
  ...
  /* ── Stage machine ── */
  // form -> uploading -> pm_ai -> pm_ai_value -> community_verify -> analyzing -> results -> saving -> posted -> error
  const [stage,      setStage]      = useState("form");

  /* ── Cloudinary upload & Answers ── */
  const [progress,   setProgress]   = useState(0);
  const [uploadErr,  setUploadErr]  = useState(null);
  const [cldResult,  setCldResult]  = useState(null);
  ...
  const startUpload = async () => {
    ...
    try {
      ...
      setCldResult(result);
      // Move to Step 2: PM AI Questions
      setStage("pm_ai");
    } catch (err) {
  ...
  /* ─────────────────────────────────────────
     STEP 2 Complete -> Step 3: AI Valuation Placeholder
  ───────────────────────────────────────── */
  const handlePmAiComplete = (collectedAnswers) => {
    setAiAnswers(collectedAnswers);
    setStage("pm_ai_value");
  };

  /* ─────────────────────────────────────────
     STEP 3 Complete -> Step 4: Community Verify Placeholder
  ───────────────────────────────────────── */
  const handleAiValueComplete = () => {
    setStage("community_verify");
  };

  /* ─────────────────────────────────────────
     STEP 4 Complete -> Step 5: AI Engine Analysis & Valuation Results
  ───────────────────────────────────────── */
  const handleCommunityVerifyComplete = () => {
    runAiAnalysis(cldResult, aiAnswers);
  };

  /* ─────────────────────────────────────────
     AI authenticity engine runner
  ───────────────────────────────────────── */
  ...
  /* ── Step 2 of 5: PM AI Welcome & Conversation ── */
  if (stage === "pm_ai") {
    return (
      <PmAiFlowScreen
        category={category}
        onComplete={handlePmAiComplete}
        onBack={() => setStage("form")}
      />
    );
  }

  /* ── Step 3 of 5: PM AI Estimated Value (Placeholder) ── */
  if (stage === "pm_ai_value") {
    return (
      <div style={placeholderStyles.container}>
        <div style={placeholderStyles.header}>
          <button style={placeholderStyles.iconBtn} onClick={() => setStage("pm_ai")}>
            <ChevronLeft size={22} color="#ffffff" />
          </button>
          <span style={placeholderStyles.headerTitle}>PM AI</span>
          <div style={placeholderStyles.stepBadge}>
        ...
        <div style={placeholderStyles.content}>
          <div style={placeholderStyles.iconBox}>💎</div>
          <h2 style={placeholderStyles.title}>AI Valuation Engine</h2>
          <p style={placeholderStyles.desc}>
            Analyzing standard market rates and attributes for <b>{title || "your item"}</b>...
          </p>
          <div style={placeholderStyles.tag}>Coming in Phase 2</div>

          <button style={placeholderStyles.primaryBtn} onClick={handleAiValueComplete}>
  ...
  /* ── Step 4 of 5: Community Verification (Placeholder) ── */
  if (stage === "community_verify") {
    return (
      <div style={placeholderStyles.container}>
        <div style={placeholderStyles.header}>
          <button style={placeholderStyles.iconBtn} onClick={() => setStage("pm_ai_value")}>
            <ChevronLeft size={22} color="#ffffff" />
          </button>
          <span style={placeholderStyles.headerTitle}>PM AI</span>
          <div style={placeholderStyles.stepBadge}>
        ...
        <div style={placeholderStyles.content}>
          <div style={placeholderStyles.iconBox}>🤝</div>
          <h2 style={placeholderStyles.title}>Community Verification</h2>
          <p style={placeholderStyles.desc}>
            Connecting to Point Market peer-verification protocol...
          </p>
          <div style={placeholderStyles.tag}>Coming in Phase 2</div>

          <button style={placeholderStyles.primaryBtn} onClick={handleCommunityVerifyComplete}>
            <span>Calculate AI Score</span>
            <ArrowRight size={18} />
          </button>
        </div>
      </div>
