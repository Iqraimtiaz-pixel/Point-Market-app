...
import { runAiAuthenticityEngine, karmaBandLabel, evaluateSkillKarma } from "../utils/aiEngine";
...
export function CreateScreen({ currentUser, userLocation }) {
  ...
  const startUpload = async () => {
    if (!title.trim() || !mediaFile) return;
    // ── Video is mandatory for Skill/Service listings — checked before any
    // upload begins, so nothing gets wasted uploading a rejected file. ──
    const needsVideo = isSkillCategory(category) || isServiceCategory(category);
    if (needsVideo && !mediaFile.type.startsWith("video/")) {
      setUploadErr("A video is required for Skill/Service listings.");
      setStage("error");
      return;
    }
    setStage("uploading");
    ...
    try {
      ...
      // ── Duration gate for Skill/Service listings: 15s–5min inclusive.
      // Checked here, right after Cloudinary returns the real duration,
      // rather than guessing from the raw file client-side. ──
      if (needsVideo) {
        const dur = Number(result.duration);
        if (!Number.isFinite(dur) || dur < 15) {
          setUploadErr("Video too short — Skill/Service listings need at least 15 seconds.");
          setStage("error");
          return;
        }
        if (dur > 300) {
          setUploadErr("Video too long — Skill/Service listings can be at most 5 minutes.");
          setStage("error");
          return;
        }
      }
  ...
  const saveToFirestore = async () => {
    ...
    // ── Product Karma = price × 0.2. Skills/Services keep the existing
    // AI-derived report.recommendedKp, unchanged this round. AI's finalScore
    // remains a separate authenticity/trust score (aiKarmaScore/karmaScore)
    // and never determines the Karma Points value for a Product. ──
    const isProductListing = !isSkillCategory(category) && !isServiceCategory(category);
    const priceNum = Number(price);
    const validPrice = (isProductListing && Number.isFinite(priceNum) && priceNum > 0) ? priceNum : null;
    const productKarma = validPrice != null ? Math.round(validPrice * 0.2) : null;

    // ── Skill/Service Karma = duration-band formula + honest heuristic
    // quality adjustment (utils/aiEngine.js, evaluateSkillKarma). Completely
    // independent from productKarma above — a listing is either a Product
    // or a Skill/Service (isProductListing), never both. ──
    const isSkillListing = isSkillCategory(category) || isServiceCategory(category);
    const skillKarma = isSkillListing
      ? evaluateSkillKarma({ videoDuration: cldResult.duration, title, desc, authenticityScore: report.finalScore })
      : null;
    ...
    const postData = {
      ...
      // AI scoring results
      aiKarmaScore:   report.finalScore,        // AI authenticity/trust score — NOT Karma Points
      karmaScore:     report.finalScore,        // same — FeedCard's authenticity badge reads this
      aiValue:        productKarma != null ? productKarma : (skillKarma != null ? skillKarma : report.recommendedKp),   // authoritative Karma Points value
      recommendedPm:  productKarma != null ? productKarma : (skillKarma != null ? skillKarma : report.recommendedKp),   // mirror of aiValue, kept for backward compatibility only
      price:          validPrice,               // raw entered PKR price — Products only, null for Skills/Services or invalid input
      // ── Community verification — Products only. Skills/Services never
      // receive these fields, so their existing immediate-award flow is
      // completely unaffected. ──
      ...(isProductListing ? { karmaStatus: "pending", verifiedBy: [] } : {}),
    ...
    try {
      ...
      setStage(isProductListing ? "verification_pending" : "posted");
  ...
  if (stage === "verification_pending") {
    ...
    const potentialKarma = Math.round(displayPrice * 0.2);
    return (
      <div className="kt-scroll">
        <div className="screen-header"><h2>Create Post</h2></div>
        <div className="cld-upload-stage">
          <div className="cld-upload-icon" style={{ background: "var(--sea-light)", width: 80, height: 80 }}>
            <Users size={42} color="#1d4ed8" />
          </div>
          <div className="cld-upload-title" style={{ color: "#1d4ed8" }}>Community Verification Pending</div>
          <div className="cld-upload-sub">Your product has passed the initial AI review. Your Karma Points will be awarded after 5 community members verify this product.</div>
          ...
          <div className="cld-media-info">
            <div className="cld-info-row"><span>Product</span><b>{title}</b></div>
            <div className="cld-info-row"><span>Price</span><b>Rs. {displayPrice.toLocaleString()}</b></div>
            <div className="cld-info-row"><span>Potential Karma Points</span><b style={{ color: "#1d4ed8" }}>{potentialKarma.toLocaleString()} KP</b></div>
            <div className="cld-info-row"><span>Verification Progress</span><b>0 / 5</b></div>
          </div>

          <div style={{ fontSize: 11.5, color: "var(--muted)", textAlign: "center", padding: "0 16px", marginTop: 10, lineHeight: 1.5 }}>
            5 genuine community members must verify this product before its Karma Points are awarded. This is separate from the AI review you just completed.
          </div>
          ...
          <button className="kt-btn" style={{ marginTop: 20, maxWidth: 280, alignSelf: "center" }} onClick={() => {
            ...
            setTitle(""); setDesc(""); setCategory(""); setPrice(""); setNeedsItem("");
  ...
  return (
    <div className="kt-scroll cp-upload-screen">
      ...
      <div className="cp-upload-body">
        ...
        {!mediaUrl ? (
          ...
          <>
            ...
            {(isSkillCategory(category) || isServiceCategory(category)) && !isVideo && (
              <div className="upload-error"><AlertTriangle size={13} /> A video is required for Skill/Service listings.</div>
            )}
          </>
        )}
        ...
        <div className="cp-fields-card">
          ...
          {!isSkillCategory(category) && !isServiceCategory(category) && (
            <div className="field-block">
              <label className="field-label">Price (PKR)</label>
              <input className="field-input" type="number" min="0" placeholder="e.g. 2500" value={price} onChange={(e) => setPrice(e.target.value)} />
            </div>
          )}
          <div className="field-block">
      ...
      <div className="cp-submit-bar">
        <button
          className="kt-btn"
          disabled={!title.trim() || !mediaFile || ((isSkillCategory(category) || isServiceCategory(category)) && !isVideo)}
          style={(!title.trim() || !mediaFile || ((isSkillCategory(category) || isServiceCategory(category)) && !isVideo)) ? { opacity: 0.4 } : {}}
          onClick={startUpload}
        >
