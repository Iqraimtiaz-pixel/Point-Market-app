    ...
    const unsub = onSnapshot(q,
      (snap) => {
        const rows = snap.docs.map((d) => {
          ...
          // ── Feature 4: mark delivered — this client just received a live
          // update for this chat. Guarded so it only writes once per new
          // update (not on every snapshot re-fire) by comparing against the
          // already-stored lastDeliveredAt for this user. ──
          const myDeliveredAt = data.lastDeliveredAt?.[myUid];
          const needsDeliveredStamp =
            data.updatedAt?.toDate &&
            (!myDeliveredAt?.toDate || myDeliveredAt.toDate().getTime() < data.updatedAt.toDate().getTime());
          if (needsDeliveredStamp) {
            setDoc(doc(db, "chats", d.id), { lastDeliveredAt: { [myUid]: serverTimestamp() } }, { merge: true })
              .catch((e) => console.warn("Failed to mark delivered:", e.message));
          }
