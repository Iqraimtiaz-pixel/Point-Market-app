...
export function ProfileScreen({ onNavigate, userLocation, currentUser, onOpenDetail, onProfileUpdate }) {
  ...
  // ── Three-dot secondary-options menu — moved off the main screen into a
  // popover. Same six MenuRow entries, same onNavigate(...) calls, just a
  // different container around them. ──
  ...
  const navigateAndClose = (dest) => { setShowMenu(false); onNavigate(dest); };
  ...
  return (
    <div className="kt-scroll">
      <div className="profile-head" style={{ position: "relative" }}>
        <button
          className="icon-btn"
        ...
        {showMenu && (
          <div
            ...
            style={{
              ...
              boxShadow: "0 12px 32px -10px rgba(15,28,40,0.28)", overflow: "hidden",
              width: 200, textAlign: "left",
            }}
          >
            <MenuRow icon={Package}     label="My Listings"  onClick={() => navigateAndClose("listings")} />
            <MenuRow icon={Bookmark}    label="Saved Items"  onClick={() => navigateAndClose("saved")} />
            <MenuRow icon={MapIcon}     label="Nearby Map"   onClick={() => navigateAndClose("map")} />
            <MenuRow icon={Wallet}      label="Wallet"       sub={walletSub} onClick={() => navigateAndClose("wallet")} />
            <MenuRow icon={Inbox}       label="Orders"       onClick={() => navigateAndClose("orders")} />
            <MenuRow icon={SettingsIcon}label="Settings"     onClick={() => navigateAndClose("settings")} />
          </div>
        )}
