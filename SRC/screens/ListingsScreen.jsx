...
export function ListingsScreen({ onBack, currentUser, onOpenDetail }) {
  ...
  const skillsCount   = listings.filter((l) => isSkillCategory(l.category)).length;
  const servicesCount = listings.filter((l) => isServiceCategory(l.category)).length;
  const productsCount = listings.filter((l) => !isSkillCategory(l.category) && !isServiceCategory(l.category)).length;
